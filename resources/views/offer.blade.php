...
<div class="flex w-full ">
    <div class="container mx-auto bg-white mt-0 px-3 xl:px-10 py-5">
        @if(count($products) == 1)
        <div class="w-full text-center text-md lg:text-xl font-light space-x-1 my-5"> {{count($products)}} product found in <strong class="font-bold"> {{$offer->offer_title}}</strong>. </div>
        @elseif(count($products))
        <div class="w-full text-center text-md lg:text-xl font-light space-x-1 my-5"> {{count($products)}} products found in <strong class="font-bold"> {{$offer->offer_title}} </strong>. </div>
        @else
        <div class="w-full text-center text-md lg:text-xl font-light space-x-1 my-5"> No products found in <strong class="font-bold"> {{$offer->offer_title}} </strong>. </div>
        @endif
            ...
            @foreach($products as $product)<div class="flex flex-col justify-start items-center space-y-1">
                <div class="flex flex-col items-center">
                    <a href="/product/{{$product->product_variant_url}}">
                        <div class="flex h-52 w-40 md:h-60 md:w-48 xl:h-60 xl:w-52 border border-light-gray bg-white rounded-md shadow hover:shadow-xl transition duration-300 ease-out-expo mb-3 p-1">
