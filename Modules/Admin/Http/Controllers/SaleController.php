...
class SaleController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('HasPermission:sale-view')->only(['fetchData', 'fetchTableData']);
    }
    public function fetchData()
    {
        $currentMonthData = DB::table('order_products')->whereMonth(
            'created_at',
            Carbon::now()->format('m')
        ...
        if ($request->limit["value"] === 30) {
            if ($request->seller["id"] === 0) {
                ...
                $graphData = DB::table('order_products')->whereMonth(
                    'created_at',
                    Carbon::now()->format('m')
                )->where('seller_id', $request->seller["id"])->where('item_received', 1)->orderBy('created_at')->get(['total_price', 'created_at']);
                foreach ($graphData as $data) {
                    $dt = new DateTime($data->created_at);
                    $data->created_at = $dt->format('d-M');
                }
                $saleItems = DB::table('order_products')->join('order_shipping_details', 'order_products.order_id', '=', 'order_shipping_details.order_id')
                    ...
                    ->whereMonth(
                        'order_products.created_at',
                        Carbon::now()->format('m')
                    )->where('seller_id', $request->seller["id"])->orderBy('created_at', 'DESC')->get();
            }
